...
export function RequestScreen({ onNav, appState, appActions }) {
  ...
  if (sent) return (
    <div style={{ minHeight: "100vh", background: COLORS.bg, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24 }}>
      ...
      <div style={{ textAlign: "center", marginBottom: 32 }}>
        ...
        <h2 style={{ color: COLORS.text, fontSize: 22, fontWeight: 800, margin: "0 0 8px" }}>Money Borrowed!</h2>
        <p style={{ color: COLORS.textMuted, fontSize: 14 }}>{contact?.full} has sent you ₹{amount}.</p>
      </div>
      <div style={{ background: COLORS.card, borderRadius: 16, padding: 20, width: "100%", maxWidth: 360, border: `1px solid ${COLORS.border}` }}>
        {[
          ["Borrowing from", contact?.full],
          ["Amount", `₹${amount}`],
          ["Tenure", `${tenure} days`],
          ["Repay by", dateStr],
          ...
          <div key={k} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: `1px solid ${COLORS.border}` }}>
            ...
            <span style={{ color: k === "If unpaid" ? COLORS.orange : COLORS.text, fontSize: 14, fontWeight: 600 }}>{v}</span>
          </div>
        ))}
      </div>
    ...
    <div style={{ paddingBottom: 100 }}>
      <div style={{ background: `linear-gradient(135deg, ${COLORS.blue} 0%, #1e3a8a 100%)`, padding: "0 20px 24px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            <h2 style={{ color: "#fff", margin: 0, fontSize: 20, fontWeight: 800 }}>Borrow Money</h2>
            <p style={{ color: "rgba(255,255,255,0.6)", margin: 0, fontSize: 12 }}>Auto-repaid on due date</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: "16px" }}>
        ...
        <div style={{ background: COLORS.card, borderRadius: 14, padding: 16, marginBottom: 12, border: `1px solid ${COLORS.border}` }}>
          <p style={{ color: COLORS.textMuted, fontSize: 13, margin: "0 0 12px" }}>Who are you borrowing from?</p>
        ...
        {amount && (
          <div style={{ background: COLORS.card, borderRadius: 14, padding: 16, marginBottom: 12, border: `1px solid ${COLORS.border}` }}>
            <p style={{ color: COLORS.text, fontWeight: 700, margin: "0 0 10px", fontSize: 14 }}>📋 Borrow Summary</p>
            {[
              ["Borrowing from", contact?.full, COLORS.blue],
            ...
            <div style={{ background: "rgba(245,158,11,0.1)", borderRadius: 8, padding: "8px 10px", marginTop: 10 }}>
              <p style={{ color: COLORS.accent, fontSize: 11, margin: 0 }}>⚡ You must approve a UPI AutoPay mandate when the request is sent. ₹{amount} will be auto-deducted from your account on {dateStr}.</p>
            </div>
          </div>
        )}
        ...
        <button 
          disabled={!amount}
          onClick={() => {
            appActions.onRequest(contact, amount, purpose, tenure);
            setSent(true);
          }}
          style={{ width: "100%", padding: 16, borderRadius: 14, border: "none", background: amount ? COLORS.blue : COLORS.textDim, color: "#fff", fontSize: 16, fontWeight: 700, cursor: amount ? "pointer" : "not-allowed" }}>
          Borrow ₹{amount} from {contact?.full}
        </button>
      </div>
    </div>
  );
